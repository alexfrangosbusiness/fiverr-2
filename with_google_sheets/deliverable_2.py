...
if __name__ == "__main__":     
    client = pygsheets.authorize(service_account_file="with_google_sheets/lively-folder-354113-abf3c40268b2.json")

    st.markdown("<h1 style='text-align: center; color: blue;'>Fiverr</h1>", unsafe_allow_html=True)

    result = st.button("Update Google Sheets")
    if result:
        client = pygsheets.authorize(service_account_file="lively-folder-354113-abf3c40268b2.json")
        spreadsht = client.open("Fiverr-slushaclaus")
        worksht = spreadsht.worksheet("title", "Rosters")
        ...
        worksht.set_dataframe(df_1, 'A2')
        worksht.set_dataframe(df_2, 'E2')
